...
// Find translateExpr signature and body
...
let letCode = code.substring(letStart, letEnd);
    ...
    let
      ...
      bindStrs = Array.concatMap (\\b -> case b of
        ...
        Rec bindings -> 
            ...
            makeLocalRec (Binding _ (Ident n) e) idx =
              ...
              in if Array.length ext.args > 0 then
                let
                  ...
                  keyword = if idx == 0 then "let rec " else "and "
                  wrapper = sName <> " = box (" <> curriedArgs <> sName <> "_tco " <> String.joinWith " " ext.args <> closes <> ")"
                in [ keyword <> sName <> "_tco " <> argStrs <> " : obj = box (" <> bodyStr <> ") ", "and " <> wrapper <> " " ]
              else
                let
                  keyword = if idx == 0 then "let rec " else "and "
          ...
          in [String.joinWith "" recStrs <> "in "]
      ) binds
    in FsIdent ("(" <> String.joinWith "" bindStrs <> printExprInline (translateExpr adtCtors newEnv currentMod body) <> ")")
  `;
