...
translateBind adtCtors currentMod = case _ of
  ...
  Rec binds -> [FsLetRec (map (\(Binding _ (Ident n) e) -> 
                  let prefix = case currentMod of
                        Just m -> m <> "_"
                        Nothing -> ""
                  in { name: sanitizeName (prefix <> n), args: [], expr: translateExpr adtCtors currentMod e }) binds)]
  where
    ...
    expandBind adtCtors currentMod (Binding _ (Ident name) val) =
      let prefix = case currentMod of
            Just m -> m <> "_"
            Nothing -> ""
      in [FsLet (sanitizeName (prefix <> name)) [] (translateExpr adtCtors currentMod val)]
...
translateLit :: Map String Int -> Maybe String -> Literal (Expr Ann) -> FsExpr
translateLit adtCtors currentMod lit = case lit of
    ...
    LitRecord props ->
      let
        mapAdd (Prop key val) acc = "(Map.add \"" <> key <> "\" (box (" <> printExprInline (translateExpr adtCtors currentMod val) <> ")) " <> acc <> ")"
...
translateExpr :: Map String Int -> Maybe String -> Expr Ann -> FsExpr
translateExpr adtCtors currentMod expr = case expr of
  ExprLit _ lit -> translateLit adtCtors currentMod lit
  ...
  ExprApp _ _ _ -> 
    ...
    in case flat.fn of
      ...
      ExprVar _ qi ->
        ...
        in case Map.lookup (sanitizeName fqName) adtCtors of
          Just arity -> generateConstructorLambda (sanitizeName fqName) arity (map (translateExpr adtCtors currentMod) flat.args)
          Nothing -> FsApp (translateExpr adtCtors currentMod flat.fn) (map (translateExpr adtCtors currentMod) flat.args)
      _ -> FsApp (translateExpr adtCtors currentMod flat.fn) (map (translateExpr adtCtors currentMod) flat.args)
  ExprCase _ exprs alts -> 
    let 
      fsExprs = map (translateExpr adtCtors currentMod) exprs
  ...
  ExprLet _ binds body -> 
    let
      bindStrs = Array.concatMap (\b -> case b of
        NonRec (Binding _ (Ident n) e) -> ["let " <> sanitizeName n <> " = " <> printExprInline (translateExpr adtCtors currentMod e) <> " in "]
        Rec bindings -> 
          let
            decls = map (\(Binding _ (Ident n) _) -> "let mutable " <> sanitizeName n <> " : obj = null in ") bindings
            assigns = map (\(Binding _ (Ident n) e) -> sanitizeName n <> " <- " <> printExprInline (translateExpr adtCtors currentMod e) <> "; ") bindings
          in [String.joinWith "" decls <> String.joinWith "" assigns]
      ) binds
    in FsIdent ("(" <> String.joinWith "" bindStrs <> printExprInline (translateExpr adtCtors currentMod body) <> ")")
  ExprUpdate _ obj props ->
    let
      mapAdd (Prop k v) prev = "(Map.add \"" <> k <> "\" (box (" <> printExprInline (translateExpr adtCtors currentMod v) <> ")) " <> prev <> ")"
    in FsIdent (Array.foldr mapAdd ("(unbox<Map<string, obj>> " <> printExprInline (translateExpr adtCtors currentMod obj) <> ")") props)
...
translateBinder adtCtors currentMod = case _ of
  ...
  BinderNamed _ (Ident name) inner ->
    FsPatRaw ("(" <> printPatternInline (translateBinder adtCtors currentMod inner) <> " as " <> sanitizeName name <> ")")

translateLitBinder :: Map String Int -> Maybe String -> Literal (Binder Ann) -> FsPattern
translateLitBinder adtCtors currentMod = case _ of
